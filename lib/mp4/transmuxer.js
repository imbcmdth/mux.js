...
VideoSegmentStream = function(track) {
  ...
  this.flush = function() {
    ...
    let oldLength = nalUnits.length;
    let droppedNals = [];

    this.getTrackInfo(nalUnits);

    nalUnits = nalUnits.filter((n, i)=> {
      if (n.nalUnitType === undefined) {
        droppedNals.push(i);
        return false;
      }
      return true;
...
if (oldLength !== nalUnits.length) {
  //console.log('nalUnits 2>', nalUnits.length, droppedNals);
}
...
//console.log('gops 1>', gops.length);

    if (gops.length && !gops[0][0].keyFrame) {
//      console.log('GETTING RID OF POOP');
      var firstGop = gops.shift();
      gops.byteLength -= firstGop.byteLength;
      gops.nalCount -= firstGop.nalCount;
      gops.duration -= firstGop.duration;
    }

    if (gops.length) {
      // save all the nals in the last GOP into the gop cache
      var poppedGop = gops.pop();
      gops.byteLength -= poppedGop.byteLength;
      gops.nalCount -= poppedGop.nalCount;
      gops.duration -= poppedGop.duration;
...
//console.log('gops 2>', gops.length);

    if (!gops.length) {
      this.resetStream_();
     // console.log('QUITTING EARLY');
      return this.trigger('done', 'VideoSegmentStream');
    }
    gops.pts = gops[0].pts;
    gops.dts = gops[0].dts;
//console.log(gops.pts, gops.dts);
/*    console.log(gops.reduce((c, f)=>c.concat(f), [])
        .reduce((c, n)=>c.concat(n), [])
        .map(n=>n.nalUnitType + " - " + n.data.byteLength));
*/
  ...
  this.getTrackInfo = function(nalUnits) {
  ...
  this.groupFramesIntoGops_ = function(frames) {
    ...
    //   // console.log('NOT KEY FRAME START');
    // }
    // console.log('\nBEFORE');
    // console.log(frames);
    // frames = this.frameCache_.concat(frames);
    // console.log('\nAFTER');
    // console.log(frames);
    ...
    for (i = 0; i < frames.length; i++) {
      ...
      if (currentFrame.keyFrame) {
        ...
        // this.frameCache_ = [currentFrame];
        // console.log(this.frameCache_);
        ...
        // if (this.frameCache_.length) {
          // this.frameCache_.push(currentFrame);
        currentGop.duration += currentFrame.duration;
        currentGop.nalCount += currentFrame.length;
        currentGop.byteLength += currentFrame.byteLength;
  ...
  this.mergeGops_ = function(start, end) {
    var gop;
    gop = start.concat(end);
    gop.byteLength = start.byteLength + end.byteLength;
    gop.duration = start.duration + end.duration;
    gop.nalCount = start.nalCount + end.nalCount;
    gop.pts = start.pts;
    gop.dts = start.dts;
    return gop;
  }
