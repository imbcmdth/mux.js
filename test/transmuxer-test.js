...
test('scales compositionTimeOffset values from milliseconds to 90kHz', function() {
  ...
  samples = boxes[0].boxes[1].boxes[2].samples;
  equal(samples.length, 3, 'generated three samples');
  equal(samples[0].compositionTimeOffset, 0 * 90, 'multiplied compositionTimeOffset by 90');
  equal(samples[1].compositionTimeOffset, 1 * 90, 'multiplied compositionTimeOffset by 90');
  equal(samples[2].compositionTimeOffset, 3 * 90, 'multiplied compositionTimeOffset by 90');
});
module('AAC Stream', {
  setup: function() {
